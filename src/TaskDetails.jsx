...
export const TaskDetails = ({ isOpen, onClose, taskData: data }) => {
  ...
  const date =
    data && data["date"] ? new Date(GetDateTimestamp(data["date"])) : null;
  ...
  const formsInRow = useMemo(() => {
    ...
    let forms = [];
    let stack = [];
    let actualForm = data.$created_on_content;

    stack.push([actualForm, 0]);

    while (stack.length > 0) {
      let [node, index] = stack.pop();

      // First time passing by the node, add formData
      if (index === 0 && node.formData) {
        forms.push(node.formData);
      }

      if (node.children && node.children[index]) {
        // Put the node back to process next child
        ...
        <ModalContent>
          <ModalHeader>
            <div className="flex flex-col w-full">
              <h1 className="text-zinc-900 tracking-wider font-normal">
                Manage Task
              </h1>
              <div className="flex h-fit pt-4 z-20 w-full justify-between  border-b border-zinc-100 pb-2 items-center">
                ...
                <div className="flex  pt-1 flex-row-reverse h-fit gap-3 items-center">
                  <span className="text-small font-light text-default-500">
                    {data.email}
                  </span>
                </div>
              </div>
            ...
            <div className="flex flex-col w-full">
              <div className="flex gap-2 pb-4 pr-4 flex-row ">
                ...
                <Button
                  ...
                  isDisabled={selectedForm == formsInRow.length - 1}
                  color={
                    selectedForm == formsInRow.length - 1
                      ? "default"
                      : "primary"
                  }
                  variant="solid"
